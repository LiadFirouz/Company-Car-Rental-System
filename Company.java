...
public class Company
{
    ...
    public boolean removeRent(Date ret)
    {
        ...
        while(!isNull(pointer))
        {
            ...
            {
                // checks the head is the one to remove, otherwise link the rents without the current rent
                if(pointer == this._head)
    ...
    public char mostCommonRate()
    {
        ...
        // saving the most rated
        int max = Math.max(Math.max(rateA, rateB),Math.max(rateC, rateD));

        // checking for the top rate and return it
        if(max == rateD)
            return RATE_D;
        if(max == rateC)
            return RATE_C;
